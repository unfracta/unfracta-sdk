...
describe("PolicyEngine.plan()", () => {
    ...
    test("LEGACY_REQUIRED → classical only", () => {
        ...
        assert.ok(plan.log.some(l => l.step === "post_quantum_policy" && l.outcome === "skipped"));
    });
    test("HYBRID_PREFERRED with PQ support → classical + PQ", () => {
        ...
        assert.ok(plan.log.some(l => l.step === "post_quantum_policy" && l.outcome === "success"));
    });
    test("HYBRID_PREFERRED without PQ support → classical only", () => {
        ...
        assert.equal(plan.doClassical, true);
        assert.equal(plan.doPostQuantum, false);
        assert.ok(plan.log.some(l => l.step === "post_quantum_policy" && l.outcome === "unsupported"));
    });
    ...
    test("Classical unsupported → no execution path", () => {
        ...
        assert.equal(plan.doPostQuantum, false);
        assert.ok(plan.log.some(l => l.step === "execution_decision" && l.outcome === "failure"));
    });
    test("Unknown policy → throws and refuses to guess", () => {
        assert.throws(() => {
            PolicyEngine.plan("UNKNOWN_POLICY", hybridCapable);
        });
    });
});
//# sourceMappingURL=policyEngine.test.js.map